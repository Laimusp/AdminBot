...
@router.message(Text(startswith='Ban', ignore_case=True), F.from_user.id.in_({1797927303, 1145153004}))
async def ban_user_handler(msg: Message, bot: Bot):
    ...
    if len(args) != 1:  # если написали айди чела
        try:
            await bot.ban_chat_member(msg.chat.id, int(args[1]))
        except ValueError:
